...
class TestStoragePathResolution:
    def test_resolve_upload_path_recovers_from_stale_prefix(self, tmp_path, monkeypatch):
        from app.config import settings
        from app.utils.storage import resolve_upload_path
        monkeypatch.setattr(settings, "upload_dir", str(tmp_path / "uploads"))
        target = settings.upload_path / "batch-1" / "file.pdf"
        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_bytes(b"%PDF-1.4")
        resolved = resolve_upload_path("app/Data/uploads/batch-1/file.pdf")
        assert resolved == target


class TestBCRSSummaryOnly:
    def test_bcrs_extracted_from_summary_area(self):
        from app.services.extractor import _extract_bcrs_amount_from_summary
        page_text = """
        Item A 2 x 12
        Item B 1 x 6
        Subtotal 61.80
        ...
        BCRS 7.20
        """
        assert _extract_bcrs_amount_from_summary(page_text, 61.80, 11.12, 80.12) == 7.20

    def test_bcrs_not_taken_from_line_description_only(self):
        from app.services.extractor import _extract_bcrs_amount_from_summary
        page_text = """
        Planner Free Apple 10% PET 0.5L x12 (BCRS) 12.71
        Planner M/Vitamin 12% PET 0.5L x12 (BCRS) 12.71
        Net Total 76.26
        VAT 13.73
        Total Due 92.39
        """
        assert _extract_bcrs_amount_from_summary(page_text, 76.26, 13.73, 92.39) is None
