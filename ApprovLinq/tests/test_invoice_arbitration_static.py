...
def test_batch_pipeline_calls_arbitration_after_rules_and_before_bcrs():
    ...
    assert 'from app.services.field_resolver import resolve_invoice_row' in src
    apply_idx = src.index('apply_saved_rule_candidates(db, batch, row, candidate_payload=r)')
    arb_idx = src.index('resolve_invoice_row(db, batch, row, r')
    bcrs_idx = src.index('decide_bcrs_split(db, batch, row, r, [row])')
