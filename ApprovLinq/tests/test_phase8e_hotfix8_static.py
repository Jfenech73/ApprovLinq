...
def test_supplier_resolver_requires_actionable_evidence():
    ...
    assert "def _supplier_candidate_is_actionable(" in src
    assert 'norm.match_method == "vat_match"' in src
...
def test_simple_extract_supplier_candidates_are_header_only():
    ...
    assert "_collect_supplier_candidates(supplier_header_text or identity_text" in fn
...
def test_supplier_guards_still_present_under_current_build():
    src = read("app/services/extractor.py")
