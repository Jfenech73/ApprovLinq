...
def test_review_rows_column_is_capped_to_preview_height_and_scrollable():
    css = (ROOT / "app/static/css/components.css").read_text(encoding="utf-8")
    js = (ROOT / "app/static/js/review.js").read_text()
    html = (ROOT / "app/static/review.html").read_text()
    assert "components.css?v=20260717e" in html
    assert "review.js?v=20260717e" in html
    ...
    assert "--review-rows-column-max-height" in css
    assert "--review-rows-scroll-max-height" in css
    assert "overflow-y: auto" in css
    assert "overscroll-behavior: contain" in css
    assert "syncRowsHeightToPreview" in js
    assert "previewCol.getBoundingClientRect().height" in js
    assert "rowScroll.style.setProperty(\"--review-rows-scroll-max-height\"" in js
    assert "rowsCol.style.setProperty(\"--review-rows-column-max-height\"" in js
    assert "img.onload = scheduleRowsHeightSync" in js
    assert "window.addEventListener(\"resize\", scheduleRowsHeightSync)" in js
    assert "syncReviewColumnHeights" not in js
    assert "_columnNaturalHeight" not in js
    assert "--review-column-height" not in js
    assert ".review-page-body{display:block;min-height:calc(100vh - 57px);height:auto;max-height:none;overflow:visible}" in js
    assert ".row-list-scroll{min-height:0;height:auto;max-height:var(--review-rows-scroll-max-height,60vh);overflow-y:auto" in js
