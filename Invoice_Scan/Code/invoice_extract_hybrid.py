...
class InvoiceExtract(BaseModel):
    ...
    supplier_name: Optional[str] = Field(default=None, description="Issuer/supplier name as written.")
    summary: Optional[str] = Field(
        default=None,
        description="5-12 word summary of what the invoice is for, based ONLY on evidence on the page.",
    )
...
def render_page_to_base64_jpeg(pdf_path: Path, page_index: int, scale: float = 2.4) -> Dict[str, str]:
...
def ocr_page_tesseract(pdf_path: Path, page_index: int, scale: float = 2.8, lang: str = "eng+mlt") -> str:
    pdf = pdfium.PdfDocument(str(pdf_path))
    page = pdf.get_page(page_index)
    img = page.render(scale=scale).to_pil()
    page.close()
...
def process_pdf(
    ...
    ordered = [
        "Invoice date", "Invoice number", "Description", "Net Amount", "Vat Amount", "Total", "Currency",
...
def main():
    ...
    ap.add_argument("--pdf", required=True, help="PDF path (1 invoice per page).")
    ap.add_argument("--out", default="invoices.xlsx", help="Output Excel file.")
    ap.add_argument("--model", default="gpt-4o-mini", help="Model name (text+vision works well).")
    ...
    ap.add_argument("--tesseract-cmd", default=None, help="Full path to tesseract.exe on Windows if needed.")
    args = ap.parse_args()
